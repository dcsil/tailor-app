...
def test_search_database_success(mock_embed, mock_files_collection):
    ...
    prefilter = {}
    postfilter = {}
    result = search_database(mock_files_collection, prompt, prefilter, postfilter)

    mock_files_collection.aggregate.assert_called_once()
    assert result == ["file1", "file2"]

@patch('usecases.text_prompt.co.embed')
def test_search_database_with_prefilter(mock_embed, mock_files_collection):
    mock_embed.return_value.embeddings.float = [0.1, 0.2, 0.3] 
    ...
    prompt = "test prompt"
    prefilter = {"category": "test_category"}
    postfilter = {}
    result = search_database(mock_files_collection, prompt, prefilter, postfilter)
