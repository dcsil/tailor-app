...
def test_get_user_files_success(mock_find_documents, client):
    mock_find_documents.return_value = [
        {"_id": "1", "filename": "test1.jpg", "description": "Test file 1", "class": "fashion illustration", "colour": "red"},
        {"_id": "2", "filename": "test2.jpg", "description": "Test file 2", "class": "texture", "colour": "green"}
    ]
...
def test_update_file_success(mock_embed, mock_update_document, mock_update_blob, mock_find_documents, client):
    ...
    # Since the description changed, the embedding should be updated
...
@patch("routes.file_routes.update_document")
@patch("routes.file_routes.co.embed")
def test_update_file_no_description_change(mock_embed, mock_update_document, mock_update_blob, mock_find_documents, client):
    mock_find_documents.return_value = [
        {
            "_id": "123", 
            "filename": "test.jpg", 
...
@patch('routes.file_routes.cohere.ClientV2.embed')
@patch('routes.file_routes.insert_document')
def test_upload_file_invalid_class(mock_insert_document, mock_embed, mock_upload_file, client):
    file, filename = create_test_file()
    data = {
        'file': (file, filename),
        'description': 'Test file description',
