...
def test_search_prompt_valid_request(mock_search, mock_get_collection, mock_get_user_id, client):
    ...
    mock_search.return_value = ['image_1', 'image_2', 'image_3'], ['url_1', 'url_2', 'url_3']
...
@patch('routes.search_routes.search_database') 
def test_regenerate_search_success(mock_search, mock_update_document, mock_find_documents, mock_get_collection, mock_get_user_id, client):
    mock_get_user_id.return_value = 'user123'
    mock_get_collection.return_value = "mocked_files_collection"
    ...
    mock_search.return_value = ['image_3', 'image_4', 'image_5'], ['url_3', 'url_4', 'url_5']
    expected_board_doc = {
        "_id": "1", 
        "queue_images": [['image_4', 'url_4'], ['image_5', 'url_5']], 
        "curr_images": [['image_1', 'url_1'], ['image_2', 'url_2'], ['image_3', 'url_3']]
    }
    ...
    assert data['user_id'] == 'user123'
    mock_update_document.assert_called_once_with("user123", "temp_boards", "1", expected_board_doc)
...
@patch('routes.search_routes.update_document')
@patch('routes.search_routes.search_database') 
def test_regenerate_search_success2(mock_search, mock_update_document, mock_find_documents, mock_get_collection, mock_get_user_id, client):
    mock_get_user_id.return_value = 'user123'
    mock_get_collection.return_value = "mocked_files_collection"
    mock_find_documents.return_value = [
        {
    ...
    data = response.get_json()
    assert data['next_image'] == ['image_3', 'url_3']
    assert data['remaining_queue_size'] == 0
    assert data['user_id'] == 'user123'
