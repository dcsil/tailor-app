...
def get_config():
    config = {"mongo_uri": os.getenv("MONGO_URI"), "db_name": os.getenv("DB_NAME")}
    ...
    if (
        ...
        and ":" not in config["mongo_uri"]
    ):
        logger.error(
            "Invalid MONGO_URI format: MongoDB Atlas connection string should include credentials"
        )
        sys.exit(1)
...
def get_user_collection(user_id, collection_type):
    if db is None:
        if testing_mode:
            logger.debug(
                f"Test mode: Simulating access to collection user_{user_id}_{collection_type}"
            )
...
def update_document(user_id, collection_type, document_id, update):
    collection = get_user_collection(user_id, collection_type)
    return collection.update_one({"_id": ObjectId(document_id)}, {"$set": update})
