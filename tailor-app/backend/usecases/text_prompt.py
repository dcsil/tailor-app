...
def search_class_group(files_collection, query_emb, group_name, classes, allocation, excluded_ids=[], postfilter={}):
    """
    ...
    try:
        ...
        project = {"$project": {"score": {"$meta": "vectorSearchScore"}, "_id": 1, "blob_url": 1, "class": 1}}
        
        # Apply post-filter if present
        if len(postfilter.keys()) > 0:
            postFilter = {"$match": postfilter}
            results = list(files_collection.aggregate([new_search_query, project, postFilter]))
        else:
...
def search_database(files_collection, prompt, postfilter = {},excluded_ids=[],topK=10,):
    ...
    class_groups = {
        ...
        "creative_inspiration": {
            "classes": ["art and film"],
            "allocation": 0.1
        }
    }

    # Normalize allocations to ensure they sum to 1
    total_allocation = sum(group_info["allocation"] for group_info in class_groups.values())
