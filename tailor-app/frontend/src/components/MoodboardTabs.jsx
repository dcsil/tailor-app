...
const MoodboardTabs = ({ img_urls, img_ids, prompt, properties }) => {
  ...
  const combineImages = async (imageUrls, maxWidth = 170, maxHeight = 300) => {
    ...
    return new Promise((resolve) => {
      canvas.toBlob(
        ...
        'image/png',
        0.8
      ) // Compress the image (quality: 0.8)
    })
  }
  ...
  return (
    <div className='h-[80vh] ml-2 p-4 bg-white border-2 border-gray-300 rounded overflow-hidden max-h-[80vh] w-full mr-4'>
      ...
      {/* Panel */}
      <div className='relative overflow-y-scroll border border-gray-200 rounded p-4 mt-4' style={{ height: 'calc(87%)' }}>
        {/* Overlay Loader */}
        {isLoading && (
        ...
        {tab === Tab.ANALYSIS ? (
          <div>
            ...
            {!isLoading && (
              <button onClick={reanalyseMoodboard} className='px-4 py-2 bg-black text-white rounded-full hover:bg-gray-600'>
                Reanalyse
              </button>
            )}
          </div>
